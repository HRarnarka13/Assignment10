...
def checkUsers(url):
	...
	try:
		if 'token' in r.json()[0]:
			print("ERROR: Response from /users includes a token, which it should not do")
		else:
			print("Responds correctly without a token when requesting users from /users")
	except ValueError:
...
checkUsers(url + '/api/users')
checkPunchcard(url + '/api/punchcards/' + companyID['company_id'])
